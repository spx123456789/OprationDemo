...
+ (UIImage *) resizedImageWithName: (NSString *) name {
    return [self resizedImageWithName:name left:0.5 height:0.5] ;
}


+ (UIImage *) resizedImageWithName: (NSString *) name left:(CGFloat) left height: (CGFloat) height {
        UIImage * image = [self imageNamed:name] ;
//    UIImage *image = UNCACHE_IMAGE(name);
    
    return  [image stretchableImageWithLeftCapWidth:image.size.width * left topCapHeight:image.size.height  * height] ;
}
